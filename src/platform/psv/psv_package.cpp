...
static bool GeneratePSVHud(const std::string& runtimeDir,
    ...
    // Per-element ranges.
    struct E { int x,y,mode; unsigned int sceneMask; int pS,pC,tS,tC,sS,sC,curTex,curX,curY; bool startVis; };
    std::vector<E> es;
    for (const auto& he : elems) {
        E e; e.x = he.screenX; e.y = he.screenY; e.mode = he.runtimeMode; e.sceneMask = he.mode4SceneMask;
    ...
    f << "typedef struct { short screenX,screenY; unsigned char mode; unsigned int sceneMask; short pieceStart,pieceCount,textStart,textCount,stopStart,stopCount; short curTex,curX,curY; unsigned char startVis; } AfnHudElem;\n";
    f << "static const AfnHudElem afn_hud_elems[" << es.size() << "] = {\n";
    for (auto& e : es)
        f << "  { " << e.x << "," << e.y << "," << e.mode << "," << e.sceneMask << "u," << e.pS << "," << e.pC
...
bool PackagePSV(const std::string& runtimeDir,
    ...
    // 1b) Emit the visual-script node graph (shared NDS/PSV codegen) -> psv_script.h.
    {
        ...
        EmitNodeScriptBodies(sf, script, blueprints, bpInstances, sprites, soundInstances);
    }
