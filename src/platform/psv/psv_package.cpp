...
bool PackagePSV(const std::string& runtimeDir,
    ...
    // 1b) Emit the visual-script node graph (shared NDS/PSV codegen) -> psv_script.h.
    {
        ...
        // Rig clip names in index order (player rig; the enemy shares it) so the AI Clips
        // node can name-resolve enemy anim clips -> drift-proof across a glTF re-sort.
        std::vector<std::string> clipNames;
        if (playerRigIdx >= 0 && playerRigIdx < (int)pspRigs.size())
            for (const auto& cl : pspRigs[playerRigIdx].clips) clipNames.push_back(cl.name);
        EmitNodeScriptBodies(sf, script, blueprints, bpInstances, sprites, soundInstances, hudNodeLayerRemap, hudNodeLayerCount, clipNames);
