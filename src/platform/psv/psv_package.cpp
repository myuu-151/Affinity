...
static bool GeneratePSVHud(const std::string& runtimeDir,
    ...
    nodeLayerRemap.clear();   // out: editor-layer flat index -> first runtime track (PlayHudAnim node remap)
    {
        ...
        for (size_t ei = 0; ei < elems.size(); ei++) {
            for (const auto& lay : elems[ei].animLayers) {
                ...
                int layFirst = -1;   // first runtime track of THIS editor-layer (for node remap)
                ...
                // runtime track (-1 if it has no animated piece item).
...
bool PackagePSV(const std::string& runtimeDir,
    ...
    std::vector<int> hudNodeLayerRemap;   // node flat-layer index -> runtime afn_hud_layer[] index
    if (!GeneratePSVHud(runtimeDir, hudElements, assets, errorMsg, hudNodeLayerRemap))
        return false;
    ...
    // 1b) Emit the visual-script node graph (shared NDS/PSV codegen) -> psv_script.h.
    {
        ...
        EmitNodeScriptBodies(sf, script, blueprints, bpInstances, sprites, soundInstances, hudNodeLayerRemap);
