...
static bool GenerateSharedMapData(const std::string& runtimeDir,
    ...
    // OBJ 2.0 light rig: any mesh carrying lights turns on runtime GL_LIGHTING,
    // which needs per-vertex normals emitted for every lit mesh (PSV only).
    bool sceneHasLights = false;
    if (isPsv)
        for (const auto& m : meshes)
            if (!m.lights.empty()) { sceneHasLights = true; break; }
    ...
    // WL()'d coords). Energy is pre-folded into the diffuse so the runtime just
    // feeds GL: point = E·(scale/4)²/(4π²) with kq=1 quadratic falloff (matches
    // Blender's diffuse response under the Standard view transform), sun = E/π
    // directional. Capped at 8 (GL_MAX_LIGHTS).
        ...
        for (size_t k = 0; k < inst.size(); k++) {
            ...
            for (const auto& L : m.lights) {
                ...
                if (L.type == 1) {   // sun: stored travel dir -> GL "toward the light", w=0
                    ...
                    float kf = L.energy * es * es / 39.478418f;    // E/(4π²), distance scale folded in
                    w.col[0] = L.r * kf; w.col[1] = L.g * kf; w.col[2] = L.b * kf;
                    w.kc = 0.01f; w.kl = 0.0f; w.kq = 1.0f;
                }
                wl.push_back(w);
            }
        }
