...
static bool GenerateSharedMapData(const std::string& runtimeDir,
    ...
    // OBJ 2.0 light rig — DYNAMIC GL path DISABLED. The custom Vita3K vitaGL
    // build renders glLight point lights as swimming garbage (2026-07-04) and
    // FFP lighting is jitter-prone on real SGX regardless. Instead the editor
    // bakes the rig into the exported per-vertex colors (mesh IR populate in
    // frame_loop.cpp) — pixel-identical for static lights, since FFP lighting
    ...
    for (size_t mi = 0; mi < meshes.size(); mi++) {
        ...
        // Per-vertex normals for GL_LIGHTING (directions — WL's uniform scale
        // doesn't change them; the runtime enables GL_NORMALIZE for instance scale).
        if (meshEmitsNormals(m)) {
            f << "static const float afn_mesh" << mi << "_nrm[" << (vc * 3) << "] = {";
            for (int v = 0; v < vc * 3; v++) { if (v % 12 == 0) f << "\n  "; f << Flt(m.normals[v]) << ","; }
            f << "\n};\n";
        }
    ...
    // ---- scene lights (OBJ 2.0 light rig -> vitaGL GL_LIGHTING, PSV only) ----
    // Each mesh's lights are placed by every instance of that mesh, through the
    // same transform chain the runtime applies to its vertices (T·Rz·Rx·Ry·S on
    // WL()'d coords). Point falloff matches Blender's diffuse response under
    // the Standard view transform — E·(N·L)/(4π²·d_blender²) — parameterized
    // for GPU-friendly magnitudes: diffuse = E·color/(4π²) (tens, not
    // thousands) and the world→Blender distance conversion lives in the
    // quadratic attenuation kq = 1/(scale/4)², NOT folded into the color.
    // (Folding it produced diffuse ~10⁴ against d² ~10⁵ — mathematically
    // identical, numerically garbage on SGX.) Sun = E/π directional. Capped
    // at 8 (GL_MAX_LIGHTS).
    if (isPsv && sceneHasLights) {
        struct WLightRow { float pos[4], col[4], kc, kl, kq; };
        std::vector<WLightRow> wl;
        float amb[3] = { 0.05f, 0.05f, 0.05f }; bool ambSet = false;
        for (size_t k = 0; k < inst.size(); k++) {
            const auto& s = sprites[inst[k]];
            const auto& m = meshes[s.meshIdx];
            if (m.lights.empty()) continue;
            if (!ambSet) { amb[0] = m.lightAmbient[0]; amb[1] = m.lightAmbient[1]; amb[2] = m.lightAmbient[2]; ambSet = true; }
            const float d2r = 3.14159265f / 180.0f;
            float cyr = cosf(s.rotation  * d2r), syr = sinf(s.rotation  * d2r);
            float cxr = cosf(s.rotationX * d2r), sxr = sinf(s.rotationX * d2r);
            float czr = cosf(s.rotationZ * d2r), szr = sinf(s.rotationZ * d2r);
            auto rot = [&](float px, float py, float pz, float* o) {
                float x1 =  cyr*px + syr*pz, y1 = py,              z1 = -syr*px + cyr*pz;  // Ry
                float x2 = x1,               y2 = cxr*y1 - sxr*z1, z2 = sxr*y1 + cxr*z1;   // Rx
                o[0] = czr*x2 - szr*y2; o[1] = szr*x2 + czr*y2; o[2] = z2;                 // Rz
            };
            for (const auto& L : m.lights) {
                WLightRow w = {};
                w.col[3] = 1.0f;
                if (L.type == 1) {   // sun: stored travel dir -> GL "toward the light", w=0
                    float d[3]; rot(L.x, L.y, L.z, d);
                    w.pos[0] = -d[0]; w.pos[1] = -d[1]; w.pos[2] = -d[2]; w.pos[3] = 0.0f;
                    float kf = L.energy / 3.14159265f;
                    w.col[0] = L.r * kf; w.col[1] = L.g * kf; w.col[2] = L.b * kf;
                    w.kc = 1.0f; w.kl = 0.0f; w.kq = 0.0f;
                } else {             // point
                    float p[3]; rot(WL(L.x) * s.scale, WL(L.y) * s.scale, WL(L.z) * s.scale, p);
                    w.pos[0] = WX(s.x) + p[0]; w.pos[1] = WY(s.y) + p[1]; w.pos[2] = WX(s.z) + p[2]; w.pos[3] = 1.0f;
                    float es = s.scale * 0.25f;                    // Blender-local -> world distance scale
                    if (es <= 1e-6f) es = 1.0f;
                    float kf = L.energy / 39.478418f;              // E/(4π²)
                    w.col[0] = L.r * kf; w.col[1] = L.g * kf; w.col[2] = L.b * kf;
                    w.kc = 0.01f; w.kl = 0.0f; w.kq = 1.0f / (es * es);  // kq·d_world² = d_blender²
                }
                wl.push_back(w);
            }
        }
        if ((int)wl.size() > 8) wl.resize(8);   // GL_MAX_LIGHTS
        if (!wl.empty()) {
            f << "#define AFN_HAS_LIGHTS 1\n";
            f << "#define AFN_LIGHT_COUNT " << wl.size() << "\n";
            f << "const int afn_light_count = " << wl.size() << ";\n";
            f << "const AfnLight afn_lights[" << wl.size() << "] = {\n";
            for (const auto& w : wl)
                f << "  { {" << Flt(w.pos[0]) << "," << Flt(w.pos[1]) << "," << Flt(w.pos[2]) << "," << Flt(w.pos[3]) << "}, {"
                  << Flt(w.col[0]) << "," << Flt(w.col[1]) << "," << Flt(w.col[2]) << "," << Flt(w.col[3]) << "}, "
                  << Flt(w.kc) << ", " << Flt(w.kl) << ", " << Flt(w.kq) << " },\n";
            f << "};\n";
            f << "const float afn_light_ambient[4] = { " << Flt(amb[0]) << ", " << Flt(amb[1]) << ", " << Flt(amb[2]) << ", 1.0f };\n\n";
        }
    }
...
static bool GenerateSharedSprites(const std::string& runtimeDir, const char* hdrPrefix,
    ...
    struct Inst { float x, y, z, scale; int asset, anim, sprite;
                  int parent; float offX, offY, offZ; int bone; int drive; int effect; };
    ...
    for (size_t i = 0; i < sprites.size(); i++) {
        ...
        if (s.assetIdx >= 0 && s.assetIdx < (int)assets.size() &&
            ...
            inst.push_back({ WX(s.x), WY(s.y), WX(s.z), s.scale, s.assetIdx, s.animIdx, (int)i,
                             s.parentIdx, WL(s.offsetX), WL(s.offsetY), WL(s.offsetZ), s.boneIdx, s.driveElementIdx, s.effectKind });
    }
    ...
    // Per-sub-sprite procedural effect: 0 = draw the sprite, >0 = draw an effect there
    // instead (1 = Aura Sphere). Lets an attached orb be a glow FX (e.g. the focus ball).
    {
        bool anyEffect = false;
        for (auto& in : inst) if (in.effect > 0) { anyEffect = true; break; }
        if (anyEffect) {
            f << "#define AFN_HAS_SPR_EFFECT 1\n";
            f << "static const int afn_spr_effect[" << inst.size() << "] = {";
            for (auto& in : inst) f << in.effect << ","; f << "};\n";
        }
    }
