    ...
    if (hasAnyScript) {
        ...
        auto emitAction = [&](const AfnScriptNodeExport* a) {
            ...
            case AfnScriptNodeType::ThrowBall: {
                ...
                auto* d4=findDataIn(a->id,4);   // Idle Clip (post-throw stance until the ball respawns)
                int clipDef = -1, idleDef = -1;
                for (size_t ci = 0; ci < clipNames.size(); ci++) {
                    std::string n = clipNames[ci];
                    for (auto& ch : n) ch = (char)tolower((unsigned char)ch);
                    if (n == "pitch" && clipDef < 0) clipDef = (int)ci;
                    if (n == "idle"  && idleDef < 0) idleDef = (int)ci;
                }
                ...
                f << "    afn_pbt_idle_clip = " << (d4?resolveInt(d4):idleDef) << ";   // stand plain-idle until the ball respawns\n";
                break;
            }
            case AfnScriptNodeType::AimBall: {
                ...
                auto* d6=findDataIn(a->id,6);  auto* d7=findDataIn(a->id,7);   // Aim Clip (held pose while aiming)
                ...
                f << "    afn_pbt_aim_clip = " << (d7?resolveInt(d7):-1) << ";   // -1 = keep the current anim while aiming\n";
                break;
            }
            ...
            case AfnScriptNodeType::BeamClash: {
                ...
                int punPct  = d10 ? resolveInt(d10) : 0;
                int punLen  = d11 ? resolveInt(d11) : 4;
                ...
                f << "    afn_clash_punish_pct = " << punPct << "; afn_clash_punish_len = " << punLen << ";\n";
                break;
            }
