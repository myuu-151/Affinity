...
static bool GeneratePSPPlayerCol(const std::string& runtimeDir,
                                 const std::vector<PSPRigExport>& pspRigs,
                                 int playerRigIdx,
                                 const std::vector<GBASpriteExport>& sprites,
                                 std::string& errorMsg) {
    std::string path = runtimeDir + "\\include\\psp_player.h";
    std::ofstream f(path);
    if (!f) { errorMsg = "Could not open " + path + " for writing"; return false; }
...
bool PackagePSP(const std::string& runtimeDir,
    ...
    if (!GeneratePSPSound(runtimeDir, soundSamples, soundInstances, errorMsg))
        return false;
    if (!GeneratePSPPlayerCol(runtimeDir, pspRigs, playerRigIdx, sprites, errorMsg))
        return false;
