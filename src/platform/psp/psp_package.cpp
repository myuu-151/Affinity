...
static bool GeneratePSPMapData(const std::string& runtimeDir,
    ...
    // Multi-texture-slot static meshes (OBJ usemtl) are a PSV-only feature; the
    // PSP runtime keeps the single-texture AfnMesh layout.
    bool isPsv = (std::string(hdrPrefix) == "psv_");
    ...
    for (size_t mi = 0; mi < meshes.size(); mi++) {
        ...
        // Multi-material meshes carry UVs even if slot 0's inline texture is unset;
        // gate UV/white-color emission on "any texture present", not just slot 0.
        bool multiMat = isPsv && !m.materials.empty();
        bool hasTex = textured || multiMat;
        ...
        for (int v = 0; v < vc; v++) {
            ...
            if (hasTex && (int)m.uvs.size() >= (v + 1) * 2) {
                u = m.uvs[v*2+0];
        ...
        if (mats > 0) {
            std::vector<std::vector<unsigned short>> slotIdx(mats);
            int triCount = (int)m.indices.size() / 3;
            for (int t = 0; t < triCount; t++) {
                int slot = t < (int)m.triMaterial.size() ? m.triMaterial[t] : 0;
                if (slot < 0 || slot >= mats) slot = 0;
                slotIdx[slot].push_back((unsigned short)m.indices[t*3+0]);
                slotIdx[slot].push_back((unsigned short)m.indices[t*3+1]);
                slotIdx[slot].push_back((unsigned short)m.indices[t*3+2]);
            }
            for (size_t q = 0; q + 4 <= m.quadIndices.size(); q += 4) {
                int qi = (int)(q / 4);
                int slot = qi < (int)m.quadMaterial.size() ? m.quadMaterial[qi] : 0;
                if (slot < 0 || slot >= mats) slot = 0;
                unsigned a=m.quadIndices[q],b=m.quadIndices[q+1],c=m.quadIndices[q+2],d=m.quadIndices[q+3];
                auto& si = slotIdx[slot];
                si.push_back((unsigned short)a); si.push_back((unsigned short)b); si.push_back((unsigned short)c);
                si.push_back((unsigned short)a); si.push_back((unsigned short)c); si.push_back((unsigned short)d);
            }
            for (int g = 0; g < mats; g++) {
                const auto& si = slotIdx[g];
                f << "static const unsigned short afn_mesh" << mi << "_idx" << g << "[" << (si.empty()?1:si.size()) << "] = {";
                for (size_t k = 0; k < si.size(); k++) { if (k % 16 == 0) f << "\n  "; f << si[k] << ","; }
                if (si.empty()) f << "0,";
                f << "\n};\n";
                const auto& M = m.materials[g];
                bool mtex = M.textured && M.texW > 0 && !M.texPixels.empty();
                if (mtex) {
                    int n = M.texW * M.texH;
                    bool opa = !m.textureHasAlpha;
                    f << "static const unsigned int __attribute__((aligned(16))) afn_mesh" << mi << "_tex" << g << "[" << n << "] = {";
                    for (int p = 0; p < n; p++) {
                        if (p % 8 == 0) f << "\n  ";
                        unsigned char pp = (p < (int)M.texPixels.size()) ? M.texPixels[p] : 0;
                        bool opaque = opa || pp != 0;
                        f << "0x" << std::hex << Rgb15ToAbgr(M.texPalette[pp], opaque) << std::dec << "u,";
                    }
                    f << "\n};\n";
                }
            }
            f << "static const unsigned short* const afn_mesh" << mi << "_idx_ptrs[" << mats << "] = {";
            for (int g = 0; g < mats; g++) f << "afn_mesh" << mi << "_idx" << g << ",";
            f << "};\n";
            f << "static const int afn_mesh" << mi << "_idx_counts[" << mats << "] = {";
            for (int g = 0; g < mats; g++) f << (int)slotIdx[g].size() << ",";
            f << "};\n";
            auto slotTex = [&](int g){ const auto& M = m.materials[g]; return M.textured && M.texW > 0 && !M.texPixels.empty(); };
            f << "static const unsigned int* const afn_mesh" << mi << "_tex_ptrs[" << mats << "] = {";
            for (int g = 0; g < mats; g++) f << (slotTex(g) ? std::string("afn_mesh")+std::to_string(mi)+"_tex"+std::to_string(g) : std::string("0")) << ",";
            f << "};\n";
            f << "static const int afn_mesh" << mi << "_tex_w[" << mats << "] = {";
            for (int g = 0; g < mats; g++) f << (slotTex(g) ? m.materials[g].texW : 0) << ",";
            f << "};\n";
            f << "static const int afn_mesh" << mi << "_tex_h[" << mats << "] = {";
            for (int g = 0; g < mats; g++) f << (slotTex(g) ? m.materials[g].texH : 0) << ",";
            f << "};\n";
        }
        f << "\n";
    }
        ...
        if (isPsv) {
            if (mats > 0)
                f << ", " << mats << ", afn_mesh" << mi << "_idx_ptrs, afn_mesh" << mi << "_idx_counts, afn_mesh"
                  << mi << "_tex_ptrs, afn_mesh" << mi << "_tex_w, afn_mesh" << mi << "_tex_h";
            else
                f << ", 0, 0, 0, 0, 0, 0";
        }
        f << " },\n";
...
static bool GeneratePSPSprites(const std::string& runtimeDir, const char* hdrPrefix,
    ...
    struct Inst { float x, y, z, scale; int asset, anim, sprite;
                  int parent; float offX, offY, offZ; int bone; int drive; };
    ...
    for (size_t i = 0; i < sprites.size(); i++) {
        ...
        if (s.assetIdx >= 0 && s.assetIdx < (int)assets.size() &&
            ...
            inst.push_back({ WX(s.x), WY(s.y), WX(s.z), s.scale, s.assetIdx, s.animIdx, (int)i,
                             s.parentIdx, WL(s.offsetX), WL(s.offsetY), WL(s.offsetZ), s.boneIdx, s.driveElementIdx });
    }
    ...
    auto emitFrameTex = [&](int ai, int fi) {
        ...
        for (int py = 0; py < h; py++)
            for (int px = 0; px < w; px++) {
                ...
                // PSV soft alpha: replace the opaque A byte with the source PNG's
                // per-pixel alpha so feathered edges blend (runtime alpha-blends).
                if (softAlpha && idx != 0)
    ...
    // Drive-through-element: -1 = normal, >=0 = run that HUD element's first anim
    // layer (rotation/scale keyframes) on this sub-sprite's own billboard.
    {
        bool anyDrive = false;
        for (auto& in : inst) if (in.drive >= 0) { anyDrive = true; break; }
        if (anyDrive) {
            f << "#define AFN_HAS_SPR_DRIVE_ELEM 1\n";
            f << "static const int afn_spr_drive_elem[" << inst.size() << "] = {";
            for (auto& in : inst) f << in.drive << ","; f << "};\n";
        }
    }
