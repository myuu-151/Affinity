...
enum class VsNodeType : int {
    ...
    // More conditions
    IsFlagSet,      // gate: passes if flag is set
    IsHPZero,       // gate: passes if sprite HP == 0
    IsNear,         // gate: passes if two sprites within radius
    // Timers/counters
    Countdown,      // decrement counter, fire when reaches 0
    ResetTimer,     // reset a timer node's counter
    Increment,      // increment variable slot by 1
    Decrement,      // decrement variable slot by 1
    // Camera
    SetFOV,         // set camera FOV
    ShakeStop,      // stop screen shake
    LockCamera,     // lock camera angle
    UnlockCamera,   // unlock camera angle
    SetCamSpeed,    // set camera follow speed
    // Physics
    ApplyForce,     // add to velocity
    Bounce,         // reverse Y velocity with damping
    SetFriction,    // set ground friction
    // Object spawning
    CloneSprite,    // duplicate a sprite at runtime
    HideAll,        // hide all sprites
    ShowAll,        // show all sprites
    // Data
    Divide,         // A / B
    Power,          // A^B (integer)
    ...
    // Batch 6: World & AI
    SetBGColor,     // set background/clear color
    GetDeltaTime,   // outputs frame delta (always 1 at 60fps)
    MapValue,       // map value from [inMin,inMax] to [outMin,outMax]
    Wrap,           // wrap value within [min, max] range
    SmoothStep,     // smoothstep interpolation
    EaseIn,         // ease-in (quadratic)
    EaseOut,        // ease-out (quadratic)
    DistToPlayer,   // distance from sprite to player
    FacePlayer,     // make sprite face toward player
    MoveForward,    // move sprite in its facing direction
    Patrol,         // move sprite between two points
    ChasePlayer,    // move sprite toward player
    FleePlayer,     // move sprite away from player
    SetAI,          // set AI behavior mode for sprite
    GetAI,          // get current AI behavior mode
    OnDeath,        // event: fires when sprite HP reaches 0
    OnHit,          // event: fires when sprite takes damage
    EmitParticle,   // spawn particle at position
    SetTint,        // set sprite color tint
    Shake,          // shake a specific sprite
    SetText,        // set HUD text slot
    ShowHUD,        // show HUD element
    HideHUD,        // hide HUD element
...
static const VsNodeTypeDef sVsNodeDefs[] = {
    ...
    // More conditions
    { "Is Flag Set",    0xFF885533, 1, 1, 1, 0, {"Flag (int)"}, {}, {} },
    { "Is HP Zero",     0xFF885533, 1, 1, 1, 0, {"Object (int)"}, {}, {} },
    { "Is Near",        0xFF885533, 1, 1, 3, 0, {"Obj A (int)", "Obj B (int)", "Radius (int)"}, {}, {} },
    // Timers/counters
    { "Countdown",      0xFF3355AA, 1, 1, 1, 0, {"Frames (int)"}, {}, {} },
    { "Reset Timer",    0xFF3355AA, 1, 1, 0, 0, {}, {}, {} },
    { "Increment",      0xFF3355AA, 1, 1, 1, 0, {"Var Slot (int)"}, {}, {} },
    { "Decrement",      0xFF3355AA, 1, 1, 1, 0, {"Var Slot (int)"}, {}, {} },
    // Camera
    { "Set FOV",        0xFF3355AA, 1, 1, 1, 0, {"FOV (int)"}, {}, {} },
    { "Shake Stop",     0xFF3355AA, 1, 1, 0, 0, {}, {}, {} },
    { "Lock Camera",    0xFF3355AA, 1, 1, 0, 0, {}, {}, {} },
    { "Unlock Camera",  0xFF3355AA, 1, 1, 0, 0, {}, {}, {} },
    { "Set Cam Speed",  0xFF3355AA, 1, 1, 1, 0, {"Speed (int)"}, {}, {} },
    // Physics
    { "Apply Force",    0xFF3355AA, 1, 1, 2, 0, {"X (int)", "Z (int)"}, {}, {} },
    { "Bounce",         0xFF3355AA, 1, 1, 1, 0, {"Damping (float)"}, {}, {} },
    { "Set Friction",   0xFF3355AA, 1, 1, 1, 0, {"Friction (float)"}, {}, {} },
    // Object spawning
    { "Clone Sprite",   0xFF3355AA, 1, 1, 1, 0, {"Source (int)"}, {}, {} },
    { "Hide All",       0xFF3355AA, 1, 1, 0, 0, {}, {}, {} },
    { "Show All",       0xFF3355AA, 1, 1, 0, 0, {}, {}, {} },
    // Data
    { "Divide",         0xFF666688, 0, 0, 2, 1, {"A", "B"}, {"Result"}, {} },
    { "Power",          0xFF666688, 0, 0, 2, 1, {"Base", "Exp"}, {"Result"}, {} },
    { "Remap",          0xFF666688, 0, 0, 3, 1, {"Value", "In Max", "Out Max"}, {"Result"}, {} },
    ...
    // Batch 6: World & AI
    { "Set BG Color",   0xFF3355AA, 1, 1, 1, 0, {"Color (int)"}, {}, {} },
    { "Get Delta Time", 0xFF666688, 0, 0, 0, 1, {}, {"DT"}, {} },
    { "Map Value",      0xFF666688, 0, 0, 4, 1, {"Value", "In Min", "In Max", "Out Min"}, {"Result"}, {} },
    { "Wrap",           0xFF666688, 0, 0, 3, 1, {"Value", "Min", "Max"}, {"Result"}, {} },
    { "Smooth Step",    0xFF666688, 0, 0, 3, 1, {"A", "B", "T"}, {"Result"}, {} },
    { "Ease In",        0xFF666688, 0, 0, 1, 1, {"T"}, {"Result"}, {} },
    { "Ease Out",       0xFF666688, 0, 0, 1, 1, {"T"}, {"Result"}, {} },
    { "Dist To Player", 0xFF666688, 0, 0, 1, 1, {"Object (int)"}, {"Dist"}, {} },
    { "Face Player",    0xFF3355AA, 1, 1, 1, 0, {"Object (int)"}, {}, {} },
    { "Move Forward",   0xFF3355AA, 1, 1, 2, 0, {"Object (int)", "Speed (int)"}, {}, {} },
    { "Patrol",         0xFF3355AA, 1, 1, 4, 0, {"Object (int)", "X1 (int)", "Z1 (int)", "Speed (int)"}, {}, {} },
    { "Chase Player",   0xFF3355AA, 1, 1, 2, 0, {"Object (int)", "Speed (int)"}, {}, {} },
    { "Flee Player",    0xFF3355AA, 1, 1, 2, 0, {"Object (int)", "Speed (int)"}, {}, {} },
    { "Set AI",         0xFF3355AA, 1, 1, 2, 0, {"Object (int)", "Mode (int)"}, {}, {} },
    { "Get AI",         0xFF666688, 0, 0, 1, 1, {"Object (int)"}, {"Mode"}, {} },
    { "On Death",       0xFF338833, 0, 1, 1, 0, {"Object (int)"}, {}, {} },
    { "On Hit",         0xFF338833, 0, 1, 1, 0, {"Object (int)"}, {}, {} },
    { "Emit Particle",  0xFF3355AA, 1, 1, 3, 0, {"Type (int)", "X (int)", "Z (int)"}, {}, {} },
...
static const char* sGMInstrumentNames[128] = {
...
struct MidiFile {
    ...
    int channelBank[16] = {};   // sample bank index per MIDI channel
    int channelProgram[16];     // GM program number per channel (-1 = unset)
    char channelName[16][48];   // display name per channel
...
// Parse Windows GM DLS soundbank for drum kit
static bool LoadGMDrumKit(SoundSample& kit) {
    ...
    // Verify RIFF DLS header
    if (sz < 12 || memcmp(d, "RIFF", 4) != 0 || memcmp(d + 8, "DLS ", 4) != 0)
        return false;

    // Step 1: Find wave pool (wvpl) — collect all wave data offsets
    struct WaveInfo {
    ...
    uint32_t wvplSize = 0;
    {
        ...
        while (p + 8 <= end) {
            ...
            if (ckId == 0x5453494C && p + 12 <= end) { // 'LIST'
                uint32_t listType = rd32(p + 8);
                if (listType == 0x6C707677) { // 'wvpl'
                    wvpl = p + 12;
                    wvplSize = ckSize - 4;
                    break;
                }
    ...
    if (!wvpl) return false;

    // Parse each wave in wvpl
    {
    ...
    // Step 2: Find percussion instrument (bank 128)
    // Look for lins LIST -> ins LIST with insh bank=128
    ...
    if (!lins) return false;

    // Find the drum instrument (bank MSB & 0x80 set, or ulBank & 0x80000000)
    const uint8_t* drumIns = nullptr;
    uint32_t drumInsSize = 0;
    {
        ...
        while (p + 12 <= lEnd) {
            ...
            if (ckId == 0x5453494C) { // LIST
                ...
                if (listType == 0x20736E69) { // 'ins '
                    // Search for insh chunk inside this instrument
                    ...
                    while (ip + 8 <= iEnd) {
                        ...
                        if (ickId == 0x68736E69 && ickSize >= 12) { // 'insh'
                            ...
                            if (ulBank & 0x80000000) { // percussion
                                drumIns = p + 12;
                                drumInsSize = ckSize - 4;
                                break;
                            }
                        }
                        ip += 8 + ((ickSize + 1) & ~1);
    ...
    if (!drumIns) return false;

    // Step 3: Parse regions (lrgn -> rgn) to map key ranges to wave indices
    const uint8_t* lrgn = nullptr;
    uint32_t lrgnSize = 0;
    {
        const uint8_t* p = drumIns;
        const uint8_t* iEnd = drumIns + drumInsSize;
        while (p + 12 <= iEnd) {
            uint32_t ckId = rd32(p);
            uint32_t ckSize = rd32(p + 4);
            if (ckId == 0x5453494C) { // LIST
                uint32_t listType = rd32(p + 8);
                if (listType == 0x6E67726C) { // 'lrgn'
                    lrgn = p + 12;
                    lrgnSize = ckSize - 4;
                    break;
                }
    ...
    snprintf(kit.name, sizeof(kit.name), "GM Drums");
    kit.isDrumKit = true;
    kit.builtIn = true;
    kit.sampleRate = 16384;
    ...
    while (rp + 12 <= rEnd) {
        ...
        if (ckId == 0x5453494C) { // LIST
            ...
            if (listType == 0x206E6772 || listType == 0x326E6772) { // 'rgn ' or 'rgn2'
                ...
                while (rrp + 8 <= rrEnd) {
                    ...
                    if (rckId == 0x686E6772 && rckSize >= 8) { // 'rgnh'
                        keyLo = rd16(rrp + 8);
                        keyHi = rd16(rrp + 10);
                    } else if (rckId == 0x6B6E6C77 && rckSize >= 12) { // 'wlnk'
                        waveIdx = (int)rd32(rrp + 16);
                    }
                    rrp += 8 + ((rckSize + 1) & ~1);
                }

                // Convert wave and store into drum hits
                if (waveIdx >= 0 && waveIdx < (int)waves.size() && waves[waveIdx].data) {
                    ...
                    for (int note = keyLo; note <= keyHi && note < 128; note++) {
                        ...
                        if (note >= 35 && note <= 81)
                            ...
                            snprintf(hit.name, sizeof(hit.name), "Drum %d", note);
                    }
                }
            }
        }
...
static void InitBuiltInSamples()
{
    ...
    // Square 75%
    addWave("Square 75%", [](int i, int len) -> int8_t {
        return (i < len * 3 / 4) ? 96 : -96;
    }, 32, false);

    // Square 12.5% (100% is just DC — 12.5% is the GBA's 4th duty)
    addWave("Square 12.5%", [](int i, int len) -> int8_t {
        return (i < len / 8) ? 96 : -96;
    }, 32, false);
    ...
    // Flute (pure tone with gentle 2nd harmonic — airy, hollow)
    addWave("Flute", [](int i, int len) -> int8_t {
        float p = 2.0f * 3.14159f * i / len;
        float v = sinf(p)
               + 0.15f * sinf(2*p)
               + 0.05f * sinf(3*p);
        return (int8_t)(v * 80.0f / 1.2f);
    }, 64, false);

    // Music Box (bell-like — fundamental + strong 3rd partial + high shimmer)
    addWave("Music Box", [](int i, int len) -> int8_t {
        float p = 2.0f * 3.14159f * i / len;
        float v = sinf(p)
               + 0.1f  * sinf(2*p)
               + 0.45f * sinf(3*p)
               ...
               + 0.2f  * sinf(5*p)
               + 0.08f * sinf(8*p);
        return (int8_t)(v * 70.0f / 1.88f);
    }, 64, false);

    // Celeste (soft bell — detuned pairs for chorus shimmer)
    addWave("Celeste", [](int i, int len) -> int8_t {
        float p = 2.0f * 3.14159f * i / len;
        float v = sinf(p)
               + 0.3f * sinf(2.005f*p)   // slightly detuned 2nd
               + 0.2f * sinf(3*p)
               + 0.15f * sinf(4.01f*p)   // slightly detuned 4th
               + 0.1f * sinf(6*p);
        return (int8_t)(v * 72.0f / 1.75f);
    }, 64, false);

    // Vibraphone (mellow bell — even harmonics dominate)
    addWave("Vibraphone", [](int i, int len) -> int8_t {
        float p = 2.0f * 3.14159f * i / len;
        float v = sinf(p)
               + 0.5f * sinf(2*p)
               + 0.08f * sinf(3*p)
               ...
               + 0.05f * sinf(5*p)
               + 0.1f * sinf(6*p);
        return (int8_t)(v * 68.0f / 2.03f);
    }, 64, false);

    // Pad (warm, round — low harmonics only, organ-like)
    addWave("Pad", [](int i, int len) -> int8_t {
        float p = 2.0f * 3.14159f * i / len;
        float v = sinf(p)
               + 0.4f * sinf(2*p)
               + 0.15f * sinf(3*p)
...
static bool ParseMidiFile(const char* path, MidiFile& out)
{
    ...
    auto rd16 = [](const uint8_t* d) -> int { return (d[0] << 8) | d[1]; };
    auto rd32 = [](const uint8_t* d) -> int { return (d[0] << 24) | (d[1] << 16) | (d[2] << 8) | d[3]; };

    // MThd header
    if (memcmp(p, "MThd", 4) != 0) return false;
    int hdrLen = rd32(p + 4);
    int format = rd16(p + 8);
    int nTracks = rd16(p + 10);
    int tpb = rd16(p + 12);
    (void)format;
    out.ticksPerBeat = tpb;
    out.tempo = 120;
    p += 8 + hdrLen;

    // Initialize channel data before parsing
    for (int i = 0; i < 16; i++) {
        out.channelBank[i] = 0;
        out.channelProgram[i] = -1;
        out.channelUsed[i] = false;
        out.channelVolume[i] = 100;
        snprintf(out.channelName[i], sizeof(out.channelName[i]), "Ch %d", i + 1);
    }
    // GM standard: channel 10 is drums
    snprintf(out.channelName[9], sizeof(out.channelName[9]), "Ch 10: Drums");

    // Extract filename for display
    {
        std::string pathStr(path);
        size_t sl = pathStr.find_last_of("/\\");
        std::string fname = (sl != std::string::npos) ? pathStr.substr(sl + 1) : pathStr;
        size_t dot = fname.find_last_of('.');
        if (dot != std::string::npos) fname = fname.substr(0, dot);
        snprintf(out.name, sizeof(out.name), "%s", fname.c_str());
    }

    for (int ti = 0; ti < nTracks && p + 8 <= end; ti++) {
        if (memcmp(p, "MTrk", 4) != 0) break;
        int trkLen = rd32(p + 4);
        const uint8_t* tp = p + 8;
        const uint8_t* trkEnd = tp + trkLen;
        if (trkEnd > end) trkEnd = end;
        p = trkEnd;

        MidiTrack trk;
        snprintf(trk.name, sizeof(trk.name), "Track %d", ti);

        int absTick = 0;
        uint8_t runningStatus = 0;

        // Track active notes for duration calc
        struct ActiveNote { int note; int channel; int tick; int velocity; };
        std::vector<ActiveNote> active;

        while (tp < trkEnd) {
            // Variable-length delta time
            int delta = 0;
            while (tp < trkEnd && (*tp & 0x80)) {
                delta = (delta << 7) | (*tp & 0x7F);
                tp++;
            }
            if (tp >= trkEnd) break;
            delta = (delta << 7) | (*tp & 0x7F);
            ...
            if (tp >= trkEnd) break;
            uint8_t status = *tp;
            if (status & 0x80) { runningStatus = status; tp++; }
            ...
            if (cmd == 0x90 || cmd == 0x80) {
                if (tp + 1 >= trkEnd) break;
                int note = tp[0] & 0x7F;
                int vel = tp[1] & 0x7F;
                tp += 2;
                if (cmd == 0x90 && vel > 0) {
                    active.push_back({note, ch, absTick, vel});
                    out.channelUsed[ch] = true;
                } else {
                    // Note off — find matching active note
                    for (int ai = (int)active.size() - 1; ai >= 0; ai--) {
                        if (active[ai].note == note && active[ai].channel == ch) {
                            MidiNote mn;
                            mn.tick = active[ai].tick;
                            mn.channel = active[ai].channel;
                            mn.note = active[ai].note;
                            mn.velocity = active[ai].velocity;
                            mn.duration = absTick - active[ai].tick;
                            trk.notes.push_back(mn);
                ...
                if (metaType == 0x51 && metaLen == 3 && tp + 3 <= trkEnd) {
                    ...
                    int usPerBeat = (tp[0] << 16) | (tp[1] << 8) | tp[2];
                    if (usPerBeat > 0) out.tempo = 60000000 / usPerBeat;
                }
                if (metaType == 0x03 && metaLen > 0 && metaLen < 31 && tp + metaLen <= trkEnd) {
                    memcpy(trk.name, tp, metaLen);
                    trk.name[metaLen] = 0;
                }
                tp += metaLen;
        ...
        for (auto& an : active) {
            MidiNote mn;
            mn.tick = an.tick;
            mn.channel = an.channel;
            mn.note = an.note;
            mn.velocity = an.velocity;
            mn.duration = absTick - an.tick;
            trk.notes.push_back(mn);
        }
...
static WAVEHDR sWaveHdrs[kAudioBufCount] = {};
static int8_t sAudioBufs[kAudioBufCount][kAudioBufSize] = {};
static AudioVoice sVoices[kMaxVoices] = {};
...
static void AudioMixBuffer(int8_t* buf, int len) {
    // Clear
    memset(buf, 0, len);

    // Advance MIDI playback
    if (sMidiPlaying && sMidiPlayIdx >= 0 && sMidiPlayIdx < (int)sMidiFiles.size()) {
        ...
        for (auto& t : mf.tracks) {
            for (auto& n : t.notes) {
                if (n.tick >= sMidiPlayTick && n.tick < newTick && !mf.channelMuted[n.channel]) {
                    ...
                    if (bankIdx >= 0 && bankIdx < (int)sSoundBank.size()) {
                        ...
                        if (smp.isDrumKit) {
                            ...
                            if (!hit.data.empty()) {
                                smpData = hit.data.data();
                                smpLen = (int)hit.data.size();
                                smpRate = hit.sampleRate;
...
static void CALLBACK AudioCallback(HWAVEOUT hwo, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2) {
    ...
    if (uMsg == WOM_DONE) {
        ...
        AudioMixBuffer((int8_t*)hdr->lpData, kAudioBufSize);
        waveOutWrite(sWaveOut, hdr, sizeof(WAVEHDR));
    }
}

static void AudioInit() {
    if (sAudioOpen) return;
    WAVEFORMATEX wfx = {};
    ...
    wfx.nBlockAlign = 1;
    wfx.nAvgBytesPerSec = kAudioSampleRate;
    if (waveOutOpen(&sWaveOut, WAVE_MAPPER, &wfx, (DWORD_PTR)AudioCallback, 0, CALLBACK_FUNCTION) != MMSYSERR_NOERROR)
        return;
    for (int i = 0; i < kAudioBufCount; i++) {
        sWaveHdrs[i].lpData = (LPSTR)sAudioBufs[i];
        sWaveHdrs[i].dwBufferLength = kAudioBufSize;
        waveOutPrepareHeader(sWaveOut, &sWaveHdrs[i], sizeof(WAVEHDR));
        memset(sAudioBufs[i], 0, kAudioBufSize);
        waveOutWrite(sWaveOut, &sWaveHdrs[i], sizeof(WAVEHDR));
    }
    sAudioOpen = true;
}
...
// For preview: expand short looping waveforms into a longer decaying buffer
static std::vector<int8_t> sPreviewBuf;

static void AudioPlaySample(int bankIdx, int note = 60) {
...
static void AudioPlayMidi(int midiIdx) {
    if (!sAudioOpen) AudioInit();
...
static void AudioShutdown() {
    if (!sAudioOpen) return;
    waveOutReset(sWaveOut);
    for (int i = 0; i < kAudioBufCount; i++)
        waveOutUnprepareHeader(sWaveOut, &sWaveHdrs[i], sizeof(WAVEHDR));
    waveOutClose(sWaveOut);
    sWaveOut = nullptr;
    sAudioOpen = false;
}
...
#else
static void AudioInit() {}
static void AudioPlaySample(int, int = 60) {}
static void AudioStopAll() {}
static void AudioPlayMidi(int) {}
static void AudioShutdown() {}
#endif
...
void FrameTick(float dt)
{
        ...
        else
        {
            ...
            auto collectActionsPlay = [&](int startNodeId) -> std::vector<const VsNode*> {
                ...
                while (!front.empty() && safety < 256) {
                    ...
                    if (an->type == VsNodeType::IsNear) {
                        // Proximity check - always pass through in editor preview
                        for (auto& lk : sVsLinks)
                            if (lk.from.nodeId == an->id && lk.from.pinType == 0 && lk.from.pinIdx == 0)
                                front.push_back(lk.to.nodeId);
                        continue;
                    }
                    if (an->type == VsNodeType::Countdown) {
                        // Countdown gate - always pass through in editor preview
                        for (auto& lk : sVsLinks)
                            if (lk.from.nodeId == an->id && lk.from.pinType == 0 && lk.from.pinIdx == 0)
                                front.push_back(lk.to.nodeId);
                        continue;
            ...
            if (sVsNodeInfoIdx >= 0 && sVsNodeInfoIdx < (int)sVsNodes.size()) {
                ...
                auto buildActionCalls = [&](const VsNode& evNode, const char* indent) -> std::string {
                    ...
                    auto editorActionSuffix = [](VsNodeType t) -> const char* {
                        ...
                        case VsNodeType::Countdown:     return "_countdown";
                        case VsNodeType::ResetTimer:    return "_reset_timer";
                        case VsNodeType::Increment:     return "_increment";
                        case VsNodeType::Decrement:     return "_decrement";
                        case VsNodeType::SetFOV:        return "_set_fov";
                        case VsNodeType::ShakeStop:     return "_shake_stop";
                        case VsNodeType::LockCamera:    return "_lock_cam";
                        case VsNodeType::UnlockCamera:  return "_unlock_cam";
                        case VsNodeType::SetCamSpeed:   return "_set_cam_speed";
                        case VsNodeType::ApplyForce:    return "_apply_force";
                        case VsNodeType::Bounce:        return "_bounce";
                        case VsNodeType::SetFriction:   return "_set_friction";
                        ...
                        case VsNodeType::SetBGColor:    return "_set_bg_color";
                        case VsNodeType::FacePlayer:    return "_face_player";
                        case VsNodeType::MoveForward:   return "_move_fwd";
                        ...
                        case VsNodeType::Shake:         return "_shake_sprite";
                ...
                // Gate conditions
                case VsNodeType::IsFlagSet:
                    editorCode = "// Gate: passes if flag is set";
                ...
                case VsNodeType::IsNear:
                    ...
                    setActionFunc(infoNode, "_is_near",
                        "    FIXED dx = g_sprites[a].wx - g_sprites[b].wx;\n"
                        "    FIXED dz = g_sprites[a].wz - g_sprites[b].wz;\n"
                        "    if (dx < 0) dx = -dx;\n"
                        "    if (dz < 0) dz = -dz;\n"
                ...
                case VsNodeType::Countdown: {
                    editorCode = "// Countdown: passes when counter hits 0";
                    char bodyBuf3[256];
                    snprintf(bodyBuf3, sizeof(bodyBuf3),
                        "    static int afn_cd_%d = %s;\n"
                        "    if (--afn_cd_%d <= 0) {\n"
                        "        afn_cd_%d = %s;\n"
                        "        /* downstream */\n"
                        "    }",
                        infoNode.id, fmtInt(infoNode.id, 0, "60"),
                        infoNode.id, infoNode.id, fmtInt(infoNode.id, 0, "60"));
                    setActionFunc(infoNode, "_countdown", bodyBuf3);
                    break;
                }
                ...
                case VsNodeType::ShakeStop:
                    editorCode = "// Stop screen shake";
                ...
                case VsNodeType::FacePlayer: {
                    ...
                    snprintf(b6, sizeof(b6),
                        ...
                        fmtInt(infoNode.id, 0, "<obj>"),
                        fmtInt(infoNode.id, 0, "<obj>"));
                    setActionFunc(infoNode, "_face_player", b6);
                    break;
                }
                case VsNodeType::MoveForward: {
                    editorCode = "// Move sprite in facing direction";
                    char b6[512];
                    snprintf(b6, sizeof(b6),
                        ...
                        fmtInt(infoNode.id, 0, "<obj>"), fmtInt(infoNode.id, 1, "<speed>"));
                    setActionFunc(infoNode, "_move_fwd", b6);
                    break;
                }
                case VsNodeType::Patrol: {
                    editorCode = "// Patrol between start and target";
                    char b6[512];
                    snprintf(b6, sizeof(b6),
                        ...
                        fmtInt(infoNode.id, 3, "<speed>"));
                    setActionFunc(infoNode, "_patrol", b6);
                    break;
                }
                case VsNodeType::ChasePlayer: {
                    ...
                    snprintf(b6, sizeof(b6),
                        ...
                        "    if (dx<0) dx=-dx; if (dz<0) dz=-dz;\n"
                        "    int dist = (dx>dz)?dx+(dz>>1):dz+(dx>>1);\n"
                        "    if (dist > 0) {\n"
                        ...
                        fmtInt(infoNode.id, 0, "<obj>"), fmtInt(infoNode.id, 1, "<speed>"),
                        fmtInt(infoNode.id, 0, "<obj>"), fmtInt(infoNode.id, 1, "<speed>"));
                    setActionFunc(infoNode, "_chase_player", b6);
                    break;
                }
                ...
                case VsNodeType::EmitParticle: {
                    ...
                    snprintf(b6, sizeof(b6),
                        ...
                        fmtInt(infoNode.id, 1, "<x>"),
                        fmtInt(infoNode.id, 2, "<z>"));
                    setActionFunc(infoNode, "_emit_particle", b6);
                    break;
                }
        ...
        if (sVsCodeWindowOpen && sVsCodeWindowNodeIdx >= 0 && sVsCodeWindowNodeIdx < (int)sVsNodes.size()) {
            ...
            if (ImGui::Begin(winTitle, &sVsCodeWindowOpen)) {
                ...
                // Function declaration (editable)
                {
                    ...
                    case VsNodeType::IsFlagSet:     suffix = "_is_flag_set"; break;
                    case VsNodeType::IsHPZero:      suffix = "_is_hp_zero"; break;
                    case VsNodeType::IsNear:        suffix = "_is_near"; break;
                    case VsNodeType::Countdown:     suffix = "_countdown"; break;
                    case VsNodeType::ResetTimer:    suffix = "_reset_timer"; break;
                    case VsNodeType::Increment:     suffix = "_increment"; break;
                    case VsNodeType::Decrement:     suffix = "_decrement"; break;
                    case VsNodeType::SetFOV:        suffix = "_set_fov"; break;
                    case VsNodeType::ShakeStop:     suffix = "_shake_stop"; break;
                    case VsNodeType::LockCamera:    suffix = "_lock_cam"; break;
                    case VsNodeType::UnlockCamera:  suffix = "_unlock_cam"; break;
                    case VsNodeType::SetCamSpeed:   suffix = "_set_cam_speed"; break;
                    case VsNodeType::ApplyForce:    suffix = "_apply_force"; break;
                    case VsNodeType::Bounce:        suffix = "_bounce"; break;
                    case VsNodeType::SetFriction:   suffix = "_set_friction"; break;
                    case VsNodeType::CloneSprite:   suffix = "_clone"; break;
                    case VsNodeType::HideAll:       suffix = "_hide_all"; break;
                    case VsNodeType::ShowAll:        suffix = "_show_all"; break;
                    case VsNodeType::Divide:        suffix = "_div"; break;
                    case VsNodeType::Power:         suffix = "_pow"; break;
                    case VsNodeType::Remap:         suffix = "_remap"; break;
                    ...
                    case VsNodeType::SetBGColor:    suffix = "_set_bg_color"; break;
                    case VsNodeType::GetDeltaTime:  suffix = "_get_dt"; break;
                    case VsNodeType::MapValue:      suffix = "_map_value"; break;
                    case VsNodeType::Wrap:          suffix = "_wrap"; break;
                    case VsNodeType::SmoothStep:    suffix = "_smoothstep"; break;
                    case VsNodeType::EaseIn:        suffix = "_ease_in"; break;
                    case VsNodeType::EaseOut:       suffix = "_ease_out"; break;
                    case VsNodeType::DistToPlayer:  suffix = "_dist_to_player"; break;
                    case VsNodeType::FacePlayer:    suffix = "_face_player"; break;
                    case VsNodeType::MoveForward:   suffix = "_move_fwd"; break;
                    ...
                    case VsNodeType::OnHit:         suffix = "_on_hit"; break;
                    case VsNodeType::EmitParticle:  suffix = "_emit_particle"; break;
                    case VsNodeType::SetTint:       suffix = "_set_tint"; break;
                    case VsNodeType::Shake:         suffix = "_shake_sprite"; break;
                    case VsNodeType::SetText:       suffix = "_set_text"; break;
                    case VsNodeType::ShowHUD:       suffix = "_show_hud"; break;
                    case VsNodeType::HideHUD:       suffix = "_hide_hud"; break;
        ...
        if (ImGui::BeginPopup("##AddNode")) {
            ...
            if (hasSearch) {
                ...
                if (ImGui::BeginMenu("Actions")) {
                    ...
                    ImGui::Separator();
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::ResetTimer].name)) addNodeAt(VsNodeType::ResetTimer);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::Increment].name)) addNodeAt(VsNodeType::Increment);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::Decrement].name)) addNodeAt(VsNodeType::Decrement);
                    ImGui::Separator();
                    ...
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::Bounce].name)) addNodeAt(VsNodeType::Bounce);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::SetFriction].name)) addNodeAt(VsNodeType::SetFriction);
                    ImGui::Separator();
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::CloneSprite].name)) addNodeAt(VsNodeType::CloneSprite);
                    ...
                    ImGui::Separator();
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::SetBGColor].name)) addNodeAt(VsNodeType::SetBGColor);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::FacePlayer].name)) addNodeAt(VsNodeType::FacePlayer);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::MoveForward].name)) addNodeAt(VsNodeType::MoveForward);
                    ...
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::SetTint].name)) addNodeAt(VsNodeType::SetTint);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::Shake].name)) addNodeAt(VsNodeType::Shake);
                    ImGui::Separator();
                ...
                if (ImGui::BeginMenu("Data")) {
                    ...
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::GetHP2].name)) addNodeAt(VsNodeType::GetHP2);
                    ImGui::Separator();
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::Divide].name)) addNodeAt(VsNodeType::Divide);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::Power].name)) addNodeAt(VsNodeType::Power);
                    ...
                    ImGui::Separator();
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::GetDeltaTime].name)) addNodeAt(VsNodeType::GetDeltaTime);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::GetRandom].name)) addNodeAt(VsNodeType::GetRandom);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::DistToPlayer].name)) addNodeAt(VsNodeType::DistToPlayer);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::GetAI].name)) addNodeAt(VsNodeType::GetAI);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::ArrayGet].name)) addNodeAt(VsNodeType::ArrayGet);
                    ImGui::Separator();
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::MapValue].name)) addNodeAt(VsNodeType::MapValue);
                    if (ImGui::MenuItem(sVsNodeDefs[(int)VsNodeType::Wrap].name)) addNodeAt(VsNodeType::Wrap);
    ...
    else if (sActiveTab == EditorTab::Sound)
    {
        ...
        ImGui::TextColored(ImVec4(0.7f, 0.8f, 1.0f, 1.0f), "Sample Bank");
        ImGui::Separator();

        // Import WAV button
        if (ImGui::Button("Import WAV", ImVec2(-1, 0))) {
#ifdef _WIN32
            char wavPath[MAX_PATH] = {};
            OPENFILENAMEA ofn = {};
            ofn.lStructSize = sizeof(ofn);
            ofn.lpstrFilter = "WAV Files\0*.wav\0All\0*.*\0";
            ofn.lpstrFile = wavPath;
            ofn.nMaxFile = MAX_PATH;
        ...
        // ---- Right panel: MIDI / Channel Assignment ----
        float rightX = vp->WorkPos.x + panelW;
        float rightW = totalW - panelW;
        ImGui::SetNextWindowPos(ImVec2(rightX, bodyY));
        ImGui::SetNextWindowSize(ImVec2(rightW, bodyH));
        ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.12f, 0.12f, 0.14f, 1.0f));
        ImGui::Begin("##SndMidi", nullptr, flags);

        ImGui::TextColored(ImVec4(0.7f, 0.8f, 1.0f, 1.0f), "MIDI Sequences");
        ImGui::Separator();

        // Import MIDI button
        if (ImGui::Button("Import MIDI", ImVec2(Scaled(120), 0))) {
#ifdef _WIN32
            char midPath[MAX_PATH] = {};
            OPENFILENAMEA ofn = {};
            ofn.lStructSize = sizeof(ofn);
            ofn.lpstrFilter = "MIDI Files\0*.mid;*.midi\0All\0*.*\0";
            ofn.lpstrFile = midPath;
            ofn.nMaxFile = MAX_PATH;
            ofn.Flags = OFN_FILEMUSTEXIST;
            if (GetOpenFileNameA(&ofn)) {
                MidiFile mf;
        ...
        for (int i = 0; i < (int)sMidiFiles.size(); i++) {
            char label[80];
            int totalNotes = 0;
            for (auto& t : sMidiFiles[i].tracks) totalNotes += (int)t.notes.size();
            snprintf(label, sizeof(label), "[%d] %s (%d BPM, %d notes)", i, sMidiFiles[i].name, sMidiFiles[i].tempo, totalNotes);
            bool sel = (sSelectedMidi == i);
            if (ImGui::Selectable(label, sel))
                sSelectedMidi = i;
        }
        ...
        // Selected MIDI details + channel assignment
        if (sSelectedMidi >= 0 && sSelectedMidi < (int)sMidiFiles.size()) {
            ...
            // Channel colors (same as piano roll)
            static const uint32_t chListColors[16] = {
                0xFF4488FF, 0xFFFF8844, 0xFF44FF88, 0xFFFF44AA,
                0xFFAAAA44, 0xFF44AAAA, 0xFFAA44FF, 0xFFFFAA44,
                0xFF8888FF, 0xFFFF8888, 0xFF88FF88, 0xFFFF88FF,
            ...
            for (int ch = 0; ch < 16; ch++) {
                ...
                // Color dot
                uint32_t col = chListColors[ch];
                ImVec4 colV((col & 0xFF) / 255.0f, ((col >> 8) & 0xFF) / 255.0f,
                            ((col >> 16) & 0xFF) / 255.0f, 1.0f);
                ImGui::PushStyleColor(ImGuiCol_Text, mf.channelMuted[ch] ? ImVec4(0.4f, 0.4f, 0.4f, 1.0f) : colV);
                ImGui::Bullet();
                ImGui::PopStyleColor();
                ImGui::SameLine();
                ...
                const char* preview = (mf.channelBank[ch] >= 0 && mf.channelBank[ch] < (int)sSoundBank.size())
                    ? sSoundBank[mf.channelBank[ch]].name : "None";
                char comboId[32];
                snprintf(comboId, sizeof(comboId), "##ch%d", ch);
                if (ImGui::BeginCombo(comboId, preview)) {
                    for (int si = 0; si < (int)sSoundBank.size(); si++) {
                        char itemLabel[48];
                        snprintf(itemLabel, sizeof(itemLabel), "[%d] %s", si, sSoundBank[si].name);
                        if (ImGui::Selectable(itemLabel, si == mf.channelBank[ch])) {
                            mf.channelBank[ch] = si;
                            sProjectDirty = true;
                        }
                        if (si == mf.channelBank[ch]) ImGui::SetItemDefaultFocus();
                    }
                    ImGui::EndCombo();
                }
                ImGui::PopItemWidth();
            ...
            // Channel colors (shared with channel list)
            static const uint32_t chColors[16] = {
                0xFF4488FF, 0xFFFF8844, 0xFF44FF88, 0xFFFF44AA,
                0xFFAAAA44, 0xFF44AAAA, 0xFFAA44FF, 0xFFFFAA44,
                0xFF8888FF, 0xFFFF8888, 0xFF88FF88, 0xFFFF88FF,
                0xFFFFFF44, 0xFF44FFFF, 0xFFCC88CC, 0xFFCCCC88
            ...
            // Timeline progress bar (draggable)
            {
                ...
                if ((hovered || held) && ImGui::IsMouseDown(0) && maxTick > 0) {
                    ...
                    if (!sMidiPlaying) {
                        if (!sAudioOpen) AudioInit();
                        sMidiPlayIdx = sSelectedMidi;
                        sMidiPlaying = true;
                    }
                }

                dl->AddRectFilled(bp, ImVec2(bp.x + barW, bp.y + barH), 0xFF1A1A1A);
                if (progress > 0.0f)
                    dl->AddRectFilled(bp, ImVec2(bp.x + barW * progress, bp.y + barH), 0xFF4488FF);
                dl->AddRect(bp, ImVec2(bp.x + barW, bp.y + barH), hovered || held ? 0xFF5599FF : 0xFF333333);
                // Playhead handle
                ...
                dl->AddText(ImVec2(bp.x + (barW - tsz.x) * 0.5f, bp.y + (barH - tsz.y) * 0.5f),
                    0xFFCCCCCC, timeBuf);
                ImGui::Spacing();
            }
